 ...
 */
object LogisticRegressionWithRegularization extends App {
  ...
  val f = new DiffFunction[DenseVector[Double]] {
    ...
    def calculate(theta: DenseVector[Double]) = {
      ...
      theta(0) = 0
      val JReg = J + (lambda / 2 * m) * (theta.t * theta)
      val gradReg: DenseMatrix[Double] = grad + ((lambda / m) * theta).asDenseMatrix.t
      (JReg, gradReg.toDenseVector)
    }
  }
